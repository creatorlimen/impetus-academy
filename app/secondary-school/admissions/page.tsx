    ...
    <>
      ...
      <Section title="Secondary admissions path." align="left">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visibleSteps.map((step) => <article key={step.title} className="surface-panel rounded-[1.75rem] p-5"><h2 className="text-xl font-semibold tracking-[-0.01em] text-card-foreground">{step.title}</h2><p className="mt-3 text-sm leading-relaxed text-muted">{step.description}</p></article>)}
        </div>
      </Section>
